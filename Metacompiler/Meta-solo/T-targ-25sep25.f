...
<----
\ Primitive completely independent   ahead, >box  end-code
...
hex 40034000    over !  cell+ \ 15 Select UART base address
...
inside: 0 uvalue ERR?       \ 45  (uspace=188/172 bytes)
...
extra: : .LOGO      ( -- )
    cr cr 19 dup spaces ." *"  cr
    cr 7 dup spaces ." *" dup spaces me count type spaces ." *"
    cr 0E spaces me2 count type
    cr cr spaces ." *" cr ;
...
inside: code SET-GPIO   ( -- )      \ GPIO 0 to 29 are inputs
...
inside: create PLL-CLK  decimal
    ( 18 ) 12 c,    30 c,    60 c,    120 c,   125 c,   132 c,  200 c,  250 c, align
\ inside: create PLL-DATA   hex
label-amsterdam  hex
    ( 3F0076) 310077 , 5A0066 , 5A0063 , 780062 , 7D0062 , 840062 , 640061 , 7D0061 ,
...
\   moon FF # movs,         \
    ...
    moon  day 4 #) str,     \ ...
    sun  hop 8 #) str,      \ Reset feedback divisor
    sun  day 8 #) str,
    sun 1 # movs,           \ SUN = 1
...
inside: : SET-UART  ( -- )  \ Initialise the UART & switch
    [ cfg> ] literal @+ h-h drop >r \ Save GPIO-pin number      cfg+4
    @+ 40038000 = negate >r \ and UART number                   cfg+8
    30 >uart                \ UART control register             cfg+8 adr
...
\ : .FAULT)     ( -- )   cr cr  ." A hardfault occurred " ;
\ ' .FAULT)  refill-vec cell+  !
...
extra: : FREEZE2    ( -- )
    frozen2  [ box> ] then   here 100 + \ Image offset 0 or 1, adjust image     roma0 rama0
    dup FFF +  3F000 and >r             \ Save rounded image length             roma0 rama0     r: rounded-length
    10000000 over 100 move  {w          \ Copy secundairy boot, open Flash      roma0 rama0     r: rounded-length
    over r@ wipe-flash                  \ Erase old image                       roma0 rama0     r: rounded-length
    over 0= if                          \ Write boot image?                     roma0 rama0     r: rounded-length
        false over 100 write-flash      \ Yes, then write new secundairy boot   roma0 rama0     r: rounded-length
    then  drop   r@ ivecs !             \ Store rounded image length up front   roma0           r: rounded-length
    dup 0= 100 and +  ivecs             \ Get roma & rama                       roma1 rama1     r: rounded-length
