...
210000D0        to HOT0    \ Hot for first core!
...
border          to RAMTOP  \ End of user RAM
...
forth: create 2LITERAL immediate NONAME
    ...
    ip  { tos } ldm,    \ 2 - Read high-part to TOS
    ip  { day } ldm,    \ 2 - Read low-part to DAY
...
label-cfg
24  125  h+h    over !  cell+ \ 14 System clock frequency1 & GPIO24 for S?
hex 40038000    over !  cell+ \ 15 Select UART
...
extra: flybuf uvalue BORDER \ 50 Border pointer
...
extra: : .LOGO      ( -- )
    cr cr 17 dup spaces ." +"  cr
    cr 7 dup spaces ." +" 1+ dup spaces me count type spaces ." +"
    cr 0B spaces me2 count type
    cr cr spaces ." +" cr ;
...
extra: code BOOT  ( -- )
    ...
    pool,
      ivecs ,           \ Reset vector location
      E000ED08 ,        \ Reset vector register (VTOR)
      ramadr: R0 ,      \ Bottom R-stack
      origin ,          \ Address of DOES-code
      amsterdam ,       \ Address of high level BOOT part
    then,
    w  { hop day } ldm, \ 3 - HOP=ivecs, DAY = reset vector register (VTOR)
    hop  day ) str,     \ 2 - Set address of VTOR
...
\ 26 bytes *WO* cold start of whole RP2040, how to do a COLD2?
extra: code COLD   ( -- )
