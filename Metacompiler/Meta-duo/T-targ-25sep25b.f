\ noForth RP2040 long -- targ = 18124 bytes -- 01jan2023 --
...
\ 30jul2024 Changed REFILL & QUIT and optimised THROW & ?STACK
...
20040000        to XRAM    \ Ram for background tasks, etc.
...
<----
\ Primitive completely independant tracer
...
forth: code UM*    ( x y -- zlo zhi )  \ Total 30 to 34 cpu cycles!
    ...
    tos sun mul,            \ 1 - 3*1  56 zhi = TOS
    sun hop mul,            \ 1 - 1*4  67
    moon day mul,           \ 1 - 3*2  67
    day hop mul,            \ 1 - 2*4  78 zlo = DAY
...
hex 40034000    over !  cell+ \ 15 Select UART 0
...
extra: flybuf uvalue FLYBUF \ 21
extra: flybuf/ uvalue FLYBUF/ \ 22
extra: R0 uvalue R0         \ 23
extra: S0 uvalue S0         \ 24
extra: tib uvalue TIB       \ 25 Terminal Input Buffer
extra: tib/ uvalue TIB/     \ 26 tib-end
extra: 07 uvalue OK         \ 27 See .OK
forth: uvariable BASE       \ 28 BASE as a value
    10 ramadr: base !
forth: uvariable STATE      \ 29 STATE as a value
forth: uvariable >IN        \ 30 >IN as a value
...
inside: 0 uvalue FP         \ 40 See FHERE = circular buffer pointer
inside: 0 uvalue HLD        \ 41 See HOLD <# #>
extra:  0 uvalue HOR        \ 42 See EMIT
{{+ extra: 0 uvalue VER }}  \ 43 See CR
forth:  0 uvalue SOURCE-ID  \ 44 see KEY EVALUATE
inside: 0 uvalue ERR?       \ 45 (uspace=194 bytes)
...
extra: flybuf uvalue BORDER \ 50 Border pointer for CHERE?
...
\ extra: : 2TUCK   2swap 2over ;  ( x1 x2 x3 x4 -- x3 x4 x1 x2 x3 x4 )
\ forth: : 2ROT    2>r 2swap 2r> 2swap ;
\ extra: : -2ROT   2swap 2>r 2swap 2r> ;
...
label-amsterdam
    pool,   ramadr: R0 ,    \ S0 = R0 + 4
            lastname ,
            label-nocatchframe  -1 ,
    then,
    hop  w )+ ldr,          \ Read R0 to HOP
    hop  hop ) ldr,
    rp 4 # subs,
    begin,
        rp 4 # add,
    ...
    =? until,               \ Frame found!
        ...
        next,
    then,
    sp  hop 4 #) ldr,       \ SP = value of S0
    sp  sp ) ldr,
    rp hop mov,             \ Restore RP
...
forth: : KEY    ( -- c )    begin  key? until  'key  execute ;
...
extra: code ?STACK     ( -- )
    ramadr: FLYBUF/ ,   \ R0 = +4, S0 = +8
    ]  true ?abort  [
code>
    w  { hop } ldm,
    day  hop 8 #) ldr,                          \ 2 - Read S0 to DAY
    sp day cmp,  u>? no if,                     \ 3 - No SP underflow?
        day  hop 4 #) ldr,                      \ 2 - Read R0 to DAY
        sp day cmp,  u>? if,                    \ 3 - No SP overflow?
            rp day cmp,  u>? no if,             \ 3 - No RP underflow?
                day  hop ) ldr,                 \ 2 - Read FLYBUF/ to DAY
...
code>
    day  w ) ldr,
    day day ) ldr,      \ 2 - Fetch stack base
...
inside: : TIDY  ( -- )  \ vectortest
    here 'emit u<   if ['] EMIT) to 'emit then
    here 'key? u<   if ['] KEY?) to 'key? then
    here 'key  u<   if ['] KEY)  to 'key  then
...
forth: : QUIT   ( -- )
    cr  begin
        ...
        [ chere  nocatchframe ! ]           \ THROW emergency entry
        to err?
        cr space ib #ib type
        ...
        if  err? origin here within
            if  err? count
                over 1 and                  \ odd string address?
                if 20 min                   \ abort"
                else
                1F and ." Msg from "        \ ?abort nfa = odd
                then
                type space
            then
...
inside: routine SYSTEM  ( -- a ) \ In:  DAY=pointer to lookup string
...
inside: code SET-GPIO   ( -- )      \ GPIO 2 to 29 are inputs
    ...
    40014004 ,      \ DAY = GPIO2 control register
code>
    w  { hop day } ldm,
    w w eors,               \ W = 0
...
\ Data for eight frequencies from 12 MHz to 250 MHz
\ Valid frequencies: 12 MHz, 30 MHz, 60 MHz, 120 MHz, 125 MHz, 132 MHz & 240 MHz
\ Higher frequencies then 132MHZ will not work on every board!
\ When using higher frequencies the core voltage must be increased!!
...
extra: : BOOT1         ( code-addr -- )
    ...
    20042000 >cmd? until    \ Sent stack pointer for core-1, succeed?
...
\ note! the address offset 108 may need adjustment
extra: : CONFIG    ( -- )    \ Initialise system clock, etc.
    4 cfg @ 0< 0= if            \ Not already booted?
        [ cfg> 2 + ] literal
        ...
        set-gpio                \ All other as input
        true  4 cfg !           \ Note cores are initialised!
        ramborder 10A + h@ <> if \ New clock?
            0 cfg 2 + h@  ramborder 10A + h! \ Yes, store new clock for second core too!
        then
        ramborder cell+ @ boot1 \ Restart the second core with 'new' data
    then
    set-uart                    \ Enable UART on GPIO0 & GPIO1
    &config catch ?abort ;      \ Add alternative configuration

\ SWITCH *WO* aug 2024
extra: : SWITCH ( -- )
    'key ['] key) > ?exit       \ Do nothing when not using built-in primitives
...
align,
label-AMSTERDAM                                         \ a(((
]   !S0  config  5 ms           \ Hardware configuration
...
extra: code BOOT  ( -- )
    ...
    pool,
      ramadr: R0 ,      \ Bottom R-stack
      origin ,          \ Address of DOES-code
      amsterdam ,       \ Address of high level BOOT part
    then,
    w  { hop sun moon } ldm, \ 5 - Read all startup data
    hop hop ) ldr,      \ 2 - Read R0
    rp hop mov,         \ 1 - Set return stack to R0
    does sun mov,       \ 1 - Set DOES register
...
\ 0 = Low, 1 = load both, note! the address offsets 120 & 10A may need adjustment
inside: : IMAGE     ( a -- +n )     FFF +  1F000 and ;
extra: : FREEZE     ( -- )      frozen  ahead [ >box ] (;)
extra: : FREEZE2    ( -- )
    frozen2  [ box> ] then  here 100 +  \ Image offset 0 or 1, adjust image     \ roma h+100
( ) [ ramadr: cfg 4 cells + ] literal >r \ Address of booted record             \ roma h+100            \r boot
...
\    6 for  2dup +!  cell+  next  2drop  boot  (;)

\ Added 13aug2023 for improved configuration files
\ forth: : [ELSE] true
...
\    s" [IF]" s<> 0= if 1- then
\  again ; immediate
\ forth: : [IF] ?exit postpone [else] ; immediate
\ forth: : [THEN] ; immediate
