...
21000000 to IVECS       \ 48 vectors & default code
210000D0 to IVECS/      \ Temporary gap for .T code
210000D0 to HOT         \ Uhere starts here
21000200 to ORIGIN      \ HERE starts here
\ ... system ...
2100F800 to FLYBUF      \ 400 bytes
2100FC00 to FLYBUF/
2100FE00 to r0          \ 200 bytes
2101FF00 to s0          \ 100 bytes
2101FF00 to TIB         \ 100 bytes
21020000 to TIB/
21020000 to BORDER      \ Systems end
21040000 to RAMTOP      \ End of user RAM
20040000 to XRAM        \ Tasker RAM
...
forth definitions
create BINARY-BUFFER      100 allot \ Secundairy boot
create UF2-BUFFER       40000 allot \ 256 kByte UF2 file buffer
create BIN-BUFFER       10000 allot \ 64 kByte binary file buffer

: TARGANO?  ( adr -- err? )         21040000 21000000 within ;
: >HOSTA?   ( ta1 -- ha2 err? )     dup 21000000 -  image +  swap targano?  ;
: >TARGA?   ( ha1 -- ta2 err? )     image -  21000000 +  dup targano? ;
...
: !NOF-NAME ( -- )
    vocs? if
        s" noForth tv# RP2040 duo "
    else
        s" noForth t# RP2040 duo "
    then  !myname ;
...
: `STRING,  ( a32 n -- )            \ Allot as counted string, needs ALIGN
    dup b, `m, ;
...
include noForth-T-asm.f

hex only forth also meta also  forth definitions
: 'VEC!     ( a v -- )  \ Build an ARM corrected jump to 'a' at address 'v'
    >r 1 or r> w! ;
...
: MYNAME2,  s" M0+ core with ARMv6-M ISA" dup b, `m, `align ;
...
\   Add TVAR pfx check
    ...
    cell+ cell+ w@ dup 1 and if \ pfx is imm?
        1+ ( aligned )          \ Align CFA
        (>body) `compile,       \ Action
        (>body) w@ w,           \ RAM address pointer or index
...
0           value #BLKS     \ Number of UF2 blocks hi image
0           value BIN#      \ Binary size
uf2-buffer value 'PTR      \ UF2 address pointer

: READ-SBOOT ( -- )     \ Open secundairy boot and image file, note size!
    s" boot-nof2.bin" r/o bin open-file \ Add secundairy boot file
    abort" File failed " >r
    r@ file-size throw d>s to bin#
    binary-buffer 1000 r@ read-file throw drop
    r> close-file throw ;
: READ-UF2HI ( -- )     \ Open secundairy boot and image file, note size!
vocs? if
    s" noforth tv# duo 2.bin" r/o bin open-file \ Add second image file
else
    s" noforth t# duo 2.bin" r/o bin open-file \ Add second image file
then
    abort" File failed " >r
    r@ file-size throw d>s to bin#
    bin-buffer 10000 r@ read-file throw drop
    r> close-file throw ;
...
: UF2-HEAD  ( blk# -- ) \ Store UF2 block header
    ...
    #blocks #blks + b!+ \ Total number of blocks
    E48BFF56  b!+ ;     \ Pico family ID

: WRITE-BLOCK   ( a1 block# -- a2 )
    ( dup . ) uf2-head          \ UF2 block header & Block addresses
...
: CALC-BLOCKS   ( -- )
    bin-buffer @ >blocks to #blks   \ Length of hi NOF
    MSTART >hosta? ?abort @ >blocks \ Length of low NOF + Sboot
...
: BUILD-UF2   ( a -- )          \ Address range to convert
    ...
    MSTART  >hosta? ?abort      \ Get image address
    #blocks 1 ?do               \ Add first UF2 image behind it
        i write-block           \ Write UF2 block 'i'
    loop  drop  ( space )
\   10021000 to 'target         \ Load second target here
    bin-buffer                  \ Get second binary
    #blocks #blks bounds ?do    \ Extend UF2 image with it
        i write-block
    loop  drop ;
...
: MAKE.UF2      ( -- )          \ Make uf2 file from binary image
    ...
    uf2-buffer to 'ptr
    read-sboot  read-uf2hi      \ Read high image
    calc-blocks                 \ Calc. binary lengths
    build-uf2
    uf2-file count              \ Get UF2 file name
    w/o bin create-file         \ (Re)define UF2 file
    abort" File failed "  >r    \ Succeeded, save FID
    uf2-buffer  #blks #blocks + 200 * \ File start & length
