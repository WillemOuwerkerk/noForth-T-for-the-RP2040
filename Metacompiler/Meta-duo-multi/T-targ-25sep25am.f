...
\ 17jul2025 Rewrote CONFIG SWITCH & EMIT) KEY) KEY?) >UART BAUD S?
...
21020000        to MSTART  \ Start of binary
...
hot 20000 -     to HOT0    \ Hot for first core!
...
border          to RAMTOP  \ End of user RAM for image-1
20041000        to XRAM    \ Ram for background tasks
...
label-amsterdam
    day  pc ) ldr,  40038000 ## \ UART0 base address
    begin,
...
label-cfg
24  125  h+h    over !  cell+ \ 14 System clock frequency
hex 40038000    over !  cell+ \ 15 Select UART 1 & GPIO24 for S?
...
extra: 20041000 uvalue XHERE \ 48 A 1024 bytes memory block start address
label-refill-vec
inside: ' noop uvalue &REFILL \ 49 Alternative REFILL token
inside: ' noop uvalue &FAULT  \ 50 Hardfault handler
extra: -1 uvalue ECHO       \ 51 Echo characters back in ACCEPT
extra: flybuf uvalue BORDER \ 52 Border pointer
...
extra: : .LOGO      ( -- )
    ...
    cr 6 dup spaces ." +" dup spaces me count type spaces ." +"
    cr 09 spaces me2 count type
    cr cr spaces ." +" cr ;
...
extra: code FIFO!      ( x -- ) \ Send data to other core
...
extra: code FIFO@      ( -- x ) \ Read data from other core
    D0000050 ,          \ FIFO status register
code>
    w  w ) ldr,
    begin,
...
extra: : CONFIG    ( -- )   \ Initialise system clock, etc.
    ...
    2DUP **bic              \ Disable UART
    2 CFG @ baud            \ Set default baudrate
    !                       \ Enable UART
    5A  0 CFG C@  pads!     \ Pullup for S?
...
extra: : SWITCH ( -- )
    ...
    hot0 3C +  dup @ >r         \ Other UART address
...
( ) while  dup sleep repeat drop \ Reset state flag
    config ( [char] A emit )  5 ms  ( [char] B emit )       \ Hardware configuration
...
extra: code BOOT  ( -- )
   ...
   then,
    w { hop day sun } ldm,  \ 3 - Read all startup data
    TP hop mov,             \ 1 - Init. task pointer
    DOES sun mov,           \ 1 - Init. DOES register
    hop hop his: r0 #) ldr, \ 2 - Read R0
    rp hop mov,             \ 1 - Set return stack to R0
    ip day mov,             \ 1 - Set IP to noForth high level
    next,                   \ 6
