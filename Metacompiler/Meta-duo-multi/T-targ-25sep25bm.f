\ noForth RP2040 long -- targ = 18124 bytes -- 01mar2023 --

<----
\ noForth T en noForth Tv ( not compact )
...
\ 01mar2024 Added coöperative multitasker primitives
\ 17apr2024 Secured SHIFTs, improved TIDY & changed COLD to generate a hardware reset
...
\ For the inspiration of the bootup code thanks to:
\ Jan Bramkamp, Trevis Bemann & Matthias Koch

Additional RAM  at 0x15000000 = 16 kbyte XIP cache
                at 0x50100000 = 4 kbyte USB buffer
---->
...
210200D0        to HOT1    \ Hot for second core!
...
20040000        to XRAM    \ Ram for background tasks
...
forth: code EXECUTE ( xt -- )  \ Perform word 'xt'     \ ok
    ...
    hop  w )+ ldr,      \ 2 - Address in CFA to HOP
...
label-amsterdam
    day  pc ) ldr,  40034000 ## \ UART0 base address
    begin,
...
forth: create 2LITERAL immediate NONAME
    ...
    ip  { tos day } ldm, \ 3 - Read 2lit TOS = High, DAY = Low
...
extra: create FOR  ( -- ) immediate NONAME
    tos 0 # cmp,  =? no if,     \ 3 - TOS not zero
...
extra: create NEXT  ( -- ) immediate NONAME
    day  rp ) ldr,              \ 2 - Get loop index from RP
    day 0 # cmp,  =? no if,     \ 3 - Not zero?
...
\
\ This UM* divides 32-bits into 16bits "numbers" before multiply
\           input           |     output
\   yhi   ylo   xhi    xlo  |   zhi    zlo
\    1     2     3      4   |    56     78
\   moon  hop   sun    day  |   tos    day
\
\ Step 1 - Split in four half words
\ Step 2 - Multiply half words
\ Step 3 - Combine half words for result
\ Using LDRH to read data from stack to registers
forth: code UM*    ( x y -- zlo zhi )  \ Total 30 to 34 cpu cycles!
    hop  sp ) ldrh,         \ 2 - 4=HOP  = xlo        (step 1)
    moon  sp 2 #) ldrh,     \ 2 - 3=MOON = xhi
    day tos 10 # lsls.mv,   \ 1 - Make y
    day 10 # lsrs,          \ 1 - 2=DAY  = ylo
    sun tos 10 # lsrs.mv,   \ 1 - 1=SUN  = yhi

    tos moon mov,           \ 1 - Copy xhi            (step 2)
    tos sun muls,           \ 1 - 3*1  56 zhi = TOS
    sun hop muls,           \ 1 - 1*4  67
    moon day muls,          \ 1 - 3*2  67
    day hop muls,           \ 1 - 2*4  78 zlo = DAY

    sun moon adds,          \ 1 - 67  intermediate    (step 3)
    cs? if,                 \ 1/2     overflow?
        hop 1 # movs,       \ 1 - 56  Add hi part of zlo
        hop 10 # lsls,      \ 1
        tos hop adds,       \ 1
    then,

    moon sun mov,           \ 1 - 56  zhi, copy 67 intermediate
    sun 10 # lsrs,          \ 1 -     Only hi 16-bits
    tos sun adds,           \ 1 -     Add to zhi

    moon 10 # lsls,         \ 1 - 78  Low intermediate to hi
    day moon adds,          \ 1 -     Add to zlo
    hop 0 # movs,           \ 1
    tos hop adcs,           \ 1 - 56  Correct zhi for overflow
    day  sp ) str,          \ 2 - 78  Save zlo
    next,                   \ 6
end-code

forth: code *      ( a b -- c )
    day  sp )+ ldr,     \ 2 - Pop integer A
    tos day muls,       \ 1 - Multiply A * B
    next,               \ 6
end-code
...
extra: tib uvalue TIB       \ 31 Terminal Input Buffer
extra: tib/ uvalue TIB/     \ 32 tib-end
extra: 07 uvalue OK         \ 33 See .OK
forth: uvariable STATE      \ 34 STATE
forth: uvariable >IN        \ 35 >IN
extra: 0 uvalue #IB         \ 36 inputbuffer len
extra: 0 uvalue IB          \ 37 formal inputbuffer adr

{{+ inside: 0 uvalue VP     \ 38 pointer to search-stack
    8 uallot                \ 39, 40 space for search-stack
inside: uvariable V0  }}    \ 41 contains current voc.

extra:  0 uvalue CREATED    \ 42 LFA of last created header in dictionary.
inside: 0 uvalue CP         \ 43 : {FLY adr fp to cp ; : FLY} adr ROMP to cp ;
inside: 0 uvalue HLD        \ 44 See HOLD <# #>
extra:  0 uvalue HOR        \ 45 See EMIT
{{+ extra: 0 uvalue VER }}  \ 46 Checksum on core
forth:  0 uvalue SOURCE-ID  \ 47 see KEY EVALUATE
extra: 20040000 uvalue XHERE \ 48 A 1024 bytes memory block start address
label-refill-vec
inside: ' noop uvalue &REFILL \ 49 Alternative REFILL token
inside: ' noop uvalue &FAULT  \ 50 Hardfault handler
extra: -1 uvalue ECHO       \ 50 Echo characters back in ACCEPT
extra: flybuf uvalue BORDER \ 51 Border pointer for CHERE?

cr cr uhere u. cr cr        \ HOT + 52
...
\               sp
\               ip
extra: code PAUSE      ( -- )      \ 29 machine cycles, is 0.232 microsec. at 125 MHz
...
\ wo 03jan2023  Key & emit working fine!
\ UART0 = 40034000, UART1 = 40038000
\ KEY?)  KEY)  EMIT)
extra: code KEY?)  ( -- f )
...
inside: code +TO(   ( x -- )    \ Store X to inline address
    ip  { day } ldm,    \ 2 - Load address & skip address
    sun  day ) ldr,     \ 2 - Read value
    sun tos adds,       \ 1 - Add x to it
...
\ create +TO.T  ' +to(T  >body ,  immediate
\ create INCR.T ' incr(T >body ,  immediate
...
\ +to#  pfx-for-tvalue +to.t      \ 1
\ incr# pfx-for-tvalue incr.t     \ 2
...
\ wo 03jan2023 Return stack tools: !R0   R?  !S0
inside: code !R0  ( -- )
    day TP mov,
    day  day his: r0 #) ldr,
    rp day mov,
...
\    2DROP   2DUP   2NIP   2OVER
\    2SWAP   2TUCK  2ROT   -2ROT
forth: code 2DROP  ( da -- )
...
\ extra: : .T    ( c -- ) t? @ if nl .do .ip .rp .sp .4th .3th .2th .tos then ;
...
code>
    w  w ) ldr,
    sun sp ) ldr,       \ x = SUN
    sun  w 68 #) str,   \ Do a signed
    tos  w 6C #) str,   \ y = TOS
    day 2 # movs,       \ Wait 8 cpu cycles for result
    begin,
        moon tos mov,   \ Save y in MOON
        day 1 # subs,
    =? until,
    tos  w 70 #) ldr,   \ q = TOS
    day  w 74 #) ldr,   \ mod = DAY
    day 0 # cmp,  =? no if,
        sun moon eors,      \ tekens
        sun 1F # asrs,      \ 0 | -1
        =? no if,
            tos 1 # subs,   \ q 1-
            day moon add,   \ mod y+
...
forth: code THROW  ( n -- )
    tos 0 # cmp,  =? if,  tos sp )+ ldr,  next,  then,
label-amsterdam
    pool,   lastname ,
...
\ extra: : NN   2 0 do
\    bl-word count 2dup upper number? ?abort drop
\    loop postpone 2literal ; immediate
...
only: code CHK     ( -- u )
    origin ,            \ Begin address
    label-core  -1 ,    \ End address
    0 ,                 \ Hold inverted checksum
code>
    w  { hop day } ldm,
    tos  sp -) str,
    tos 0 # movs,
...
\ Calculate checksum over Forth core only!
\ only: : CHK  ( -- checksum )
\   origin [ chere ] literal  [ label-core ]  crc -crc xor ;
...
extra: : .LOGO      ( -- )
    cr cr 15 dup spaces ." *"  cr
    cr 5 dup spaces ." *" dup spaces me count type spaces ." *"
    cr 0A spaces me2 count type
    cr cr spaces ." *" cr ;

extra: : GPIO!     ( mode io -- )  \ GPIO-pin to mode
    2* cells 40014004 + ! ; \ IO_BANK0_BASE     control register
extra: : PADS!     ( ctrl io -- )  \ GPIO-pin PAD-control
    cells 4001C004 + ! ;    \ PADS_BANK0_BASE   pad control registers


\ ************* Warm start of noForth **************

\ Reset core-1
extra: code RESET1 ( -- )      \ 26 bytes
...
extra: code RXF?       ( -- f ) \ Data in RX fifo?
    D0000050 ,
code>
    day 1 # movs,       \ Fifo RX? bit
label-amsterdam
    tos sp -) str,      \ Save TOS
    w  w ) ldr,         \ Read addr. fifo status to W
    tos w ) ldr,        \ Read fifo status to TOS
    tos day ands,       \ Test it
    =? no if,           \ Not zero?
        tos day day subs.mv, \ Build true flag
        tos tos mvns,
    then,
    next,
end-code
extra: code TXF?       ( -- f ) \ Space in TX fifo?
    D0000050 ,
code>
...
extra: code EMPTY-FIFO ( -- )   \ Empty incoming FIFO completely
...
inside: : >CMD?         ( cmd -- f )    dup fifo!  fifo@ = ;

\ Core-1 access sequence: 0, 0, 1, vectortable, sp, pc
extra: : BOOT1         ( code-addr -- )
    ...
    21020000 >cmd? until    \ Sent interrupt table for core-1, succeed?
    2103FE00 >cmd? until    \ Sent stack pointer for core-1, succeed?
...
label-amsterdam             \ Out: SUN=rom subroutine address
    ...
    sp  day ) ldrh,         \ Read lookup code to R1
    w 0 # movs,             \ ROM start address
    ip  w 14 #) ldrh,       \ Read lookup table address
    w  w 18 #) ldrh,        \ Read address of lookup subroutine
    w blx,                  \ Call lookup table
...
code>
    ...
    moon { sp } ldm,        \ Address end
    moon { ip } ldm,        \ Address start
    amsterdam bl,           \ Get ROM function address
    sun blx,                \ Call ROM function
    { ip sp tos hop } pop,  \ Pop Forth registers
    sp 8 # adds,            \ Pop SP
...
inside: code SET-GPIO   ( -- )      \ GPIO 2 to 29 are inputs
    D0000020 ,      \ HOP = GPIO output enable
    40014004 ,      \ DAY = GPIO0 control register
code>
    w  { hop day } ldm,
    w w eors,               \ W = 0
    w  hop ) ldr,           \ GPIO all inputs
    w 1E # movs,            \ GPIO's to initialise
    sun 5 # movs,           \ GPIO's are input
    begin,
        sun  day ) str,     \ Set input
        day 8 # adds,       \ To next CTRL register
        w 1 # subs,
    =? until,
...
inside: code CLK-ON     ( div offset -- )
    4000B000 ,      \ HOP = Clocks base clear alias
    40008000 ,      \ DAY = Clocks base
    800 ,           \ SUN = Kill clock bit mask
code>
    w  { hop day sun } ldm,
...
\ From RP2040 datesheet page 426 ff.
\ Now more flexible to work for both UART's
inside: : BAUD   ( baud -- ) \ Set baudrate for UART0
    10 * >r  [ cfg> 2 + ]   \ Get system clock freq.
    literal h@  F4240 *
...
\ Data for six frequencies from 12 MHz to 250 MHz
\ Valid freq: 12MHz, 30MHz, 60MHz, 120MHz, 125MHz, 132MHz, 200MHz & 250MHz
\ Higher frequencies then 132MHZ will not work on every board!
\ Option: using higher frequencies the core voltage must be increased!
\ This is field VSEL in VREG_AND-CHIP_RESET register
\ Note that!! the >PLL code needs to be slightly rewritten...
inside: create PLL-CLK  decimal
\   0       1       2       3       4       5       6       7
    12 c,   30 c,   60 c,   120 c,  125 c,  132 c,  200 c,  250 c,  align
label-amsterdam  hex
    310077 , 5A0066 , 5A0063 , 780062 , 7D0062 , 840062 , 640061 , 7D0061 ,

\ This word is secure, uses always a valid frequency
inside: : >PLL      ( clk -- post mul ) \ Leave data for PLL settings
    0 begin
        2dup pll-clk + c@ <>        \ Invalid clock?
    while
    1+ dup 8 = until                \ Leave here when MHZ is invalid!
        drop  7D  [ cfg> 2 + ] literal  h!  4 \ Use default of 125MHz
    then  nip  cells
    [ amsterdam ] literal           \ Leave setup data
    + @ h-h >r  0C lshift  r> ;     \ Make PLL-setup factors

\ Register aliases: 1000=Xor, 2000=Set, 3000=Clear
inside: code RESTART-DEVICES ( -- )
    4000C000 ,  \ Resets base address (DAY)
    4000E000 ,  \ Resets base SET-address (SUN)
    01FFCDBF ,  \ Set most Pico devices in reset mode (MOON)
    4000F000 ,  \ Resets base CLEAR-address, part-2: (HOP)
    003C7FFE ,  \ Get devices out of reset mode. part-2: (MOON)
code>
    w  { day sun moon } ldm,
    moon  sun ) str,        \ Set most built in devices in reset state
    w  { hop moon } ldm,    \ HOP = 4000F000, MOON = 3C7FFE
    moon  hop ) str,        \ Restart most built in devices
    begin,
        w  day 8 #) ldr,    \ Read reset done register to W
        w w mvns,           \ Invert reset flags
        w moon ands,        \ Zero when all devices are ready?
    =? until,
    rotterdam 12 + 77 again, \ next,
end-code

inside: code START-XOSC     ( -- )
    40008000 ,  \ Clocks base address (HOP)
    40024000 ,  \ X-oscillator base address (DAY)
    00FABAA0 ,  \ X-osc enable pattern (SUN)
    4000B000 ,  \ Clocks base CLEAR-address  (MOON)
code>
    w  { hop day sun moon } ldm,
    w 0 # movs,             \ Disable resus mode
    w  hop 78 #) str,
    w 2F # movs,            \ Init. Xosc, set down counter
    w  day 0C #) str,
    ...
    w 1 # movs,             \ Select PLL as clock reference
    w  moon 3C #) str,
    begin,
    ...
    w 3 # movs,             \ Select PLL-usb as auxillary clock
    w  moon 30 #) str,
    begin,
...
inside: code INIT-PLLS      ( post mul -- )  \ Initialise PLL's
    40028000 ,  \ PLL sys base address
    4002C000 ,  \ PLL USB sys base address
    52000 ,     \ Clock setting for USB-PLL = 48MHz
code>
    w { hop day } ldm,      \ HOP = SYS base, DAY = USB base
    sun 0 # movs,           \ SUN = 0
    moon sun 1 # subs.mv,   \ MOON = -1
    moon  hop 4 #) str,     \ Save PLL power
    moon  day 4 #) str,
    sun  hop 8 #) str,      \ Feedback divisor
    sun  day 8 #) str,
    sun 1 # movs,           \ SUN = 1
    sun  hop ) str,         \ Divide XT by 1
    sun  day ) str,

    tos  hop 8 #) str,      \ SYS = 12 x mul = 12*mul MHz
    sun 28 # movs,          \ USB - 12 x 40 = 480 MHz
    sun  day 8 #) str,

    sun 21 # movs,          \ Start both PLL's
    sun sun mvns,
    sun  hop 4 #) str,
    sun  day 4 #) str,
    begin,                  \ Both PLL's locked?
        sun  hop ) ldr,
        moon  day ) ldr,
        sun moon ands,
    neg? until,             \ When highest bit is set

    w { moon } ldm,         \ MOON = 52000
...
inside: code INIT-CLKS    ( -- )
    40008000 ,  \ Clocks base address, W & SUN are scratch
    4000B000 ,  \ Clocks base clear alias
code>
    w  { hop day } ldm,     \ HOP = 40008000, DAY = 4000B000
    sun  hop 30 #) ldr,     \ Read clock ref ctrl to SUN
    w 2 # movs,             \ Xosc = clock source
    sun w orrs,
    w 4 # subs,             \ Mask only low two bits
    sun w ands,
    sun  hop 30 #) str,
    begin,                  \ Wait for selected source (Xosc)
        sun  hop 38 #) ldr,
        sun 4 # cmp,
    =? until,
    sun 6 # lsls,
    ...
    w 8 # lsls,             \ Divide system clock by 1, W = 0100
    w  hop 40 #) str,
    rotterdam 12 + 77 again, \ next,
end-code

\    dm 24  dm 125  0 cfg ! \ GPIO pin number for S? & default system clock
...
inside: : SET-UART  ( -- )  \ Initialise the UART & switch
    301  30 >uart           \ UART control register & data
    2dup **bic              \ Uart disable
    2 cfg @ baud            \ Set default baudrate
    !   set-gpio            \ Enable UART & All GPIO's
    2 0 gpio!  2 1 gpio!    \ Enable all uart pins
    2 4 gpio!  2 5 gpio!
    5A 0 cfg c@ pads! ;     \ All other as input & pullup for S?

extra: : CONFIG    ( -- )    \ Initialise system clock, etc.
    4 cfg @ 0< 0= >r  r@ if     \ Not already booted?
        [ cfg> 2 + ] literal
        h@  dup >PLL            \ Get PLL clock data
        ...
        true  4 cfg !           \ Note cores are initialised!
        ramborder 10A + h@ <> if \ New clock data?
            0 cfg 2 + h@  ramborder 10A + h! \ Yes, store new clock for second core too!
        then
    then
    set-uart
    &config catch ?abort        \ Add alternative configuration
    r> if
        0A ms  ramborder cell+ @ boot1 \ Restart the second core with 'new' data
    then ;
...
( ) main  begin @ dup main <>   \ Not done?
( ) while  dup sleep repeat drop \ Reset state flag
    config  5 ms                \ Hardware configuration
...
extra: code BOOT  ( -- )
    ...
    pool,
      tp> ,                 \ Main TCB
      amsterdam ,           \ Address of high level BOOT part
    then,
    w { hop moon } ldm,     \ 3 - Read all startup data
    hop hop his: r0 #) ldr, \ 2 - Read R0
    rp hop mov,             \ 1 - Set return stack to R0
    ip moon mov,            \ 1 - Set IP to noForth high level
...
dup  IVECS 3 cells +  vec! \ Hard Fault vector
     IVECS 4 cells +  vec! \ MPU fault vector


extra: : SWITCH ( -- )      \ Make other core accessible
    'key ['] tkey > ?exit       \ Do nothing when not using built-in primitives
    state @ ?abort              \ Only for use while executing!
    1 cfg dup @                 \ My UART address
    hot1 3C +  dup @ >r         \ Other UART address
    !  r> swap ! ; immediate    \ Exchange UART addresses

\ For noForth we save the whole image in Flash and we reserve
\ space for two images of 0x42000 bytes each. Stored as:
\ The bootable file is stored in Flash from XIP address: 10000000
\
\ |  0 - 100  | 1000 to 41000 | to 81000 | 81000 to 200000
\ | Bootblock | Boot image-0  | image-1  | Free for NOF
\ Image-0: 00000 .... 21000 ..... 41000 including secundairy boot
\ Image-1: 41000 .... 61000 ..... 81000
\
\ So for NOF there is a space of: #1.552.384 bytes to #16.232.448 free.
\ Depending on the size of the Flash chips that ranges from 2 Mbyte to 16 Mbyte
\ 0 = Low, 1 = load & boot both, note the address offsets: 10A & 120
inside: : IMAGE     ( a -- +n )     FFF +  FF000 and ; \ Was 1F000
extra: : FREEZE     ( -- )      frozen  ahead [ >box ] (;)
extra: : FREEZE2    ( -- )
    frozen2  [ box> ] then  here 100 +  \ Image offset 0 or 1, adjust image
( ) [ cfg> 4 cells + ] literal >r       \ Address of booted record              \ roma h+100            \r boot
( ) 1  r@  !                            \ Clear booted flag
    dup IMAGE >r                        \ Round image-0 to next erase page      \ roma h+100            \r boot len
    10000000 over 100 move  {w          \ Save secundairy boot, open Flash      \ roma h+100            \r boot len
    over r@ wipe-flash                  \ Erase old image-0                     \ roma h+100            \r boot len
    over 0= if                          \ Is it the boot image?                 \ roma h+100            \r boot len
        false over 100 write-flash      \ Yes, then write new secundairy boot   \ roma h+100            \r boot len
    then  drop
    dup 0= 100 and >r   2r@ - ivecs !   \ Store rounded image length up front   \ roma                  \r boot len
    dup r> +  ivecs                     \ Get roma & rama                       \ roma roma+ rama       \r boot len
    r@ 100 - write-flash   r> +         \ Write image-0                         \ roma2                 \r boot
( ) true  r>  !                         \ Place booted flag back                \ roma2                 \r -
\   120 + @ == HERE of other system, 10A + h! == Clock of other core
( ) ramborder  dup 120 + @ over - IMAGE >r \ Round image-1 to next erase page   \ roma2 rama2           \r len2r
( ) over r@ wipe-flash                  \ Erase image-1                         \ roma2 rama2           \r len2r
( ) false cfg 2 + h@  over 10A + h!     \ Copy clock freq. to image-1
( ) r@ over !  r> write-flash  w} ;     \ Write image-1 with rounded length in front

\ 26 bytes *WO* cold start of whole RP2040
extra: code COLD   ( -- )
    40010008 ,          \ Watchdog reset select
    40058000 ,          \ Watchdog control
    00008000 ,          \ Reset core-0 pattern
code>
    w  { hop day sun } ldm, \ Read pool
    sun  hop ) str,     \ Store core-1 reset
    sun 10 # lsls,      \ Make & activate force reset bit
    sun  day ) str,
    next,
end-code
\ extra: code COLD    10000047 ,  code> hop w ) ldr, hop bx,  end-code

extra: code COLD2   100000AB ,  code> hop w ) ldr, hop bx,  end-code
...
( ) CHERE FF + 1FF00 and  mstart !  \ Rounded image length in front
( ) CHERE   ramadr: dp !
( ) UHERE   ramadr: uhere !
\ - - -
( ) CHERE  core !
( ) DO-CHK  invert dup u.  core 4 + ! \ Finish checksum
( ) DO-CHK u.
