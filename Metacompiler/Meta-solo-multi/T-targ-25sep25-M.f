...
\ 09apr2025 Added vector for CONFIG in cell 5 of CFG table
...
\ 09jun2025 Rewrote hardfault handler, to save IP, PC & TOS and use QUIT to return
...
mstart 40000 +  to RAMTOP  \ End of user RAM for image-0
...
label-cfg
24  125  h+h    over !    cell+ \ 14 GPIO24 for S? & System clock frequency
hex 40034000    over !    cell+ \ 15 Select UART0, UART1 = 40038000
...
extra: tib uvalue TIB       \ 36 Terminal Input Buffer
extra: tib/ uvalue TIB/     \ 37 tib-end
extra: 07 uvalue OK         \ 38 See .OK
forth: uvariable STATE      \ 39 STATE flag
forth: uvariable >IN        \ 40 >IN pointer
extra: 0 uvalue #IB         \ 41 inputbuffer len
extra: 0 uvalue IB          \ 42 formal inputbuffer adr

{{+ inside: 0 uvalue VP     \ 43 pointer to search-stack
    8 uallot                \ 44, 45 space for search-stack
inside: uvariable V0 }}     \ 46 contains current voc.

extra:  0 uvalue CREATED    \ 47 LFA of last created header in dictionary.
inside: 0 uvalue CP         \ 48 : {FLY adr fp to cp ; : FLY} adr ROMP to cp ;
inside: 0 uvalue HLD        \ 49 See HOLD <# #>
extra:  0 uvalue HOR        \ 50 See EMIT
{{+ extra: 0 uvalue VER }}  \ 51 See CR
forth:  0 uvalue SOURCE-ID  \ 52 see KEY EVALUATE
extra: xram uvalue XHERE    \ 53 A 1024 bytes memory block start address
label-refill-vec
inside: ' noop uvalue &REFILL \ 54 Alternative REFILL token
inside: ' noop uvalue &FAULT  \ 55 Hardfault handler
extra: -1 uvalue ECHO       \ 56 Echo characters back in ACCEPT
extra: flybuf uvalue BORDER \ 57 Border pointer
...
inside: code !R0   ( -- )   \ Empty return stack
    day TP mov,
    day  day his: r0 #) ldr,
    rp day mov,
    next,               \ 6
...
inside: code !S0   ( -- ) \ Empty data stack
    day TP mov,
    sp  day his: s0 #) ldr,
    next,               \ 6
end-code
...
\   moon { sp ip } ldm,     \ SP = end, IP = start
...
\ Valid freq: 12MHz, 30MHz, 48, 60MHz, 120MHz, 125MHz, 132MHz, 200MHz & 250MHz
...
\   0        1        2        3        4        5        6        7        8
    12 c,    30 c,    48 c,   60 c,    120 c,   125 c,   132 c,   200 c,   250 c, align
label-amsterdam  hex
    310077 , 5A0066 , 600064 , 5A0063 , 780062 , 7D0062 , 840062 , 640061 , 7D0061 ,

\ This word is secure, uses always a valid frequency
\ Replaces the system clock when it is not a valid frequency
inside: : >PLL      ( clk -- post mul ) \ Leave data for PLL settings
    0 begin
        2dup pll-clk + c@ <>        \ Invalid clock?
    while
    1+ dup 9 = until                \ Leave here when MHZ is invalid!
        drop  7D [ cfg> 2 + ] literal  h!  5 \ Use default of 125MHz
    then  nip  cells
...
\   54000 ,     \ Clock setting for USB-PLL /40 = 48MHz
    52000 ,     \ Clock setting for USB-PLL /20 = 48MHz
code>
    w { hop day } ldm,      \ HOP = SYS base, DAY = USB base
    sun 0 # movs,           \ SUN = 0
    moon sun 1 # subs.mv,   \ MOON = -1
    moon  hop 4 #) str,     \ PWR       - Save PLL power
    moon  day 4 #) str,     \
    sun  hop 8 #) str,      \ FBDIV_INT - Feedback divisor
    sun  day 8 #) str,      \
    sun 1 # movs,           \ SUN = 1
    sun  hop ) str,         \ CS        - Divide XT by 1
    sun  day ) str,

    tos  hop 8 #) str,      \ SYS = 12 x mul = 12*mul MHz
\   sun 50 # movs,          \ USB - 12 x 80 = 960 MHz
    ...
    w { moon } ldm,         \ MOON = 52000 = /20
    tos sp )+ ldr,          \ TOS  = post clock
    tos  hop 0c #) str,     \ PRIM      - Set PLL post dividers
    moon  day 0c #) str,
    w 0 # movs,
    w  hop 4 #) str,        \ PWR       -
    w  day 4 #) str,
...
inside: : SET-UART  ( -- )  \ Initialise the UART & switch
    [ cfg> ] literal @+ h-h drop >r \ Save GPIO-pin number
    @+ 40038000 = negate >r \ and UART number
    30 >uart                \ UART control register
( ) 301 over **bic          \ Uart disable
    swap @ baud             \ Set default baudrate
    301 swap !  set-gpio    \ Enable UART & All GPIO's
    r> 2* 2*  2 over gpio!  \ UART on GPIO0/1 or GPIO4/5
    2 swap 1+ gpio!
    5A r> pads! ;           \ All other as input & pullup for S?
...
extra: : CONFIG    ( -- )   \ Initialise system clock, etc.
    [ cfg> 2 + ] literal  h@ >PLL \ Get PLL clock data
...
\ wo 19may2023  **Rename this word to: BOOT**
extra: code BOOT    ( -- )
...
\ space for two images of 0x40000 bytes each. Stored as:
\ The bootable file is stored in Flash from XIP address: 10000000
\
\ |  0 - 100  | 0100 to 41000 | to 81000 | 81000 to 200000
...
extra: : FREEZE2    ( -- )
    frozen2  [ box> ] then              \ Image offset 0 or 1
    here 100 +                          \ Adjust image
    dup FFF +  3F000 and >r             \ Save rounded image length
...
extra: : ROUTINE   ( <name> -- syscode )
    create  ( here cell- ) false 55  \ Leave address of CFA & sec. code
    {{+ assembler  }} ;
